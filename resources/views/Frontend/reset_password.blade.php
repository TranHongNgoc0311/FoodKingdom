...
<body>
	...
	<div class="container-login100" style="background-image: url({{url('public/images/bg_1.jpg')}});">
		<div class="wrap-login100 p-l-55 p-r-55 p-t-80 p-b-30 ftco-animate">
			...
			@if(!empty($token))
			<form class="login100-form validate-form" action="{{route('update_password',['token' => $token])}}" method="POST">
				@csrf
			...
			@else
			<form class="login100-form validate-form" action="{{route('send_token')}}" method="POST">
				<span class="login100-form-title p-b-37">
					Chúng tôi cần biết mail của bạn!
				</span>
				@csrf
				<div class=" m-b-20">
					<div class="wrap-input100 validate-input">
						<input class="input100" type="email" name="email" placeholder="Email của bạn là...">
						<span class="focus-input100"></span>
					</div>
					@if($errors->has('email'))
					<div class="help-block text-danger text-center">
